...
contract('Voting', (accounts) => {
  ...
  it('should set the correct owner', async () => {
    ...
    expect(contractOwner).to.be.equal(accounts[0]);
  });

  /**
   * @dev Check workflow status.
   */
  it('should not allow to tally votes before end of the voting session', async () => {
    await expectRevert(votingInstance.tallyVotes(), 'Current status is not voting session ended');
  });

  it('should not allow to end votes before it started', async () => {
    await expectRevert(votingInstance.endVotingSession(), 'Voting session havent started yet');
  });

  it('should not allow to start votes before end of proposal phase', async () => {
    await expectRevert(
  ...
  it('should not allow to end proposals before it started', async () => {
    await expectRevert(
  ...
  /**
   * @dev Now workfow are tested, continue with the other phases.
   */
  ...
  it('should properly register a voter', async () => {
    ...
    /* Adding a new voter from future tests. */
  ...
  /* TODO check you can't change status if not in proper phase. */
